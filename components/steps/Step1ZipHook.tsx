...
import { ArrowRight, ArrowLeft, ShieldCheck, Wind, Zap, Sparkles, AlertCircle } from 'lucide-react';
...
export function Step1ZipHook({ onSuccess, initialZip = '' }: Step1Props) {
  ...
  if (isOutOfStateMode) {
    return (
      <div className="animate-fade-in text-center">
        ...
        <div className="mt-6 mb-2 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 border border-amber-300 text-amber-900 text-xs font-bold">
          <AlertCircle className="w-3.5 h-3.5" />
          <span>Florida Properties Only</span>
        </div>

        <h1 className="mt-3 mb-3 font-sans text-[22px] sm:text-[26px] font-bold text-[#0F172A] leading-tight">
        ...
        <form onSubmit={handleSubmit} className="max-w-sm mx-auto space-y-4">
          ...
          <button
            ...
            className="w-full h-[52px] bg-[#8B1122] hover:bg-[#730C1A] active:scale-[0.98] disabled:opacity-40 text-white font-sans font-bold text-[15px] tracking-wide rounded-xl transition-all flex items-center justify-center cursor-pointer"
          >
            GET RESULTS
          </button>
          ...
          <button
            type="button"
            onClick={() => { setIsOutOfStateMode(false); setError(null); }}
            className="text-sm font-medium text-[#94A3B8] hover:text-[#0F172A] transition-colors cursor-pointer"
          >
            ← Go back to start
          </button>
        </form>
  ...
  return (
    <div className="animate-fade-in text-center">
      ...
      {/* Original High-Converting Headline */}
      <h1 className="font-sans text-[24px] sm:text-[28px] md:text-[30px] font-extrabold text-[#0F172A] leading-[1.2] tracking-[-0.02em] mb-2.5 max-w-lg mx-auto">
        Protect Your Home & Lower Your Florida Insurance with a Certified Roof.
      </h1>

      {/* Original High-Converting Subheadline */}
      <p className="text-[14px] sm:text-[15px] font-medium text-[#475569] leading-normal mb-8 max-w-md mx-auto">
        Skip the endless calls. Compare free quotes from Florida DBPR-licensed pros in 60 seconds.
      </p>

      {/* Zip Form */}
      <form onSubmit={handleSubmit} className="max-w-sm mx-auto space-y-3.5">
        <div>
          <input
            type="text"
            inputMode="numeric"
        ...
        <button
          type="submit"
          disabled={isSubmitting || zip.length < 5}
          className="w-full h-[52px] bg-[#8B1122] hover:bg-[#730C1A] active:scale-[0.98] disabled:opacity-40 text-white font-sans font-bold text-[15px] tracking-wide rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
          <span>CHECK LOCAL PRICING & PROS</span>
          <ArrowRight className="w-4.5 h-4.5" />
        </button>

        <p className="text-xs font-semibold text-[#0F172A] flex items-center justify-center gap-1.5 pt-1">
      ...
      <div className="mt-8 pt-6 border-t border-slate-100 max-w-md mx-auto">
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="py-2.5 px-1 rounded-xl bg-[#F8FAFC] border border-slate-200/70">
